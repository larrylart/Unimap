...
static void free_vote_matrix(double **matrix, int n);
#ifdef DEBUG
...
{
   int i=0, nbright=0, min=0;
   int num_stars_A=0;          /* number of stars in chain A */
   int num_stars_B=0;          /* number of stars in chain B */
   int num_triangles_A=0;      /* number of triangles formed from chain A */
   int num_triangles_B=0;      /* number of triangles formed from chain B */
   int **vote_matrix; 
   ...
   if (min < start_pairs)
	{
		shError("atFindTrans: only %d stars in list(s), require at least %d", min, start_pairs);
                       ...
                       RECALC_NO, max_iter, halt_sigma, trans) != SH_SUCCESS)
   {
      ...
      free_star_array(star_array_B);
		...
		// larry - free matrix
		free_vote_matrix( vote_matrix, nSBright );
...
atMatchLists
   ...
   char *basename,          /* I: base of filenames used to store the */
                            ...
                            /*      .unB    items from B that didn't match */
	int *num_matches,       /* O: number of matching pairs we find */
	s_star *star_array_REM_J,
	int *num_stars_REM_J,
	s_star *star_array_REM_K,
	int *num_stars_REM_K
   )
{
   s_star *star_array_A;
   ...
   int num_stars_B;
	...
	s_star *star_array_L, *star_array_M;
    ...
    *    basename.mtB    stars from list B that did match         array K
    *    basename.unA    stars from list A that did NOT match     array L
    *    basename.unB    stars from list B that did NOT match     array M
    */
...
*/
	...
	//////////////////////////////////
	// larry :: now we have to populate here the list a with matches
	*num_stars_REM_J = num_stars_J;
	for( i=0; i<num_stars_J; i++ )
	{
//		s_star* pStar = GetStarFromListById( listA, star_array_J[i].id );
//		if( pStar != NULL ) pStar->match_id = star_array_K[i].id;
		// also populate J remote
		star_array_REM_J[i].id = star_array_J[i].id;
		star_array_REM_J[i].x = star_array_J[i].x;
		star_array_REM_J[i].y = star_array_J[i].y;
		star_array_REM_J[i].mag = star_array_J[i].mag;

	}
	// and now we populate K
	*num_stars_REM_K = num_stars_K;
	for( i=0; i<num_stars_K; i++ )
	{
		star_array_REM_K[i].id = star_array_K[i].id;
		star_array_REM_K[i].x = star_array_K[i].x;
		star_array_REM_K[i].y = star_array_K[i].y;
		star_array_REM_K[i].mag = star_array_K[i].mag;
	}
   ...
   free_star_array(star_array_M);
	...
	// larry :: free other arrays as well
	free_star_array( star_array_A );
	free_star_array( star_array_B );
...
/////////////////////////////////////
// larry's get star by id method
s_star* GetStarFromListById( s_star* vectList, int id )
{
	s_star* pFoundStar;
	s_star* pStar;
...
{
   int i=0, nbright=0, min=0, ret=0;
   int num_stars_A=0;          /* number of stars in set A  */
   int num_stars_B=0;          /* number of stars in set B */
   static int num_triangles_A=0;  /* number of triangles formed from set A */
   int **vote_matrix; 
   int *winner_votes;        /* # votes gotten by top pairs of matched stars */
   int *winner_index_A;      /* elem i in this array is index in star array A */
                             /*    which matches ... */
...
free_vote_matrix
   (
   int** matrix,    /* I: pointer to 2-D array to be freed */
   int n              /* I: number of elements in each row and col */
   )
{
   int i;
...
{
   int i=0, j=0;
   int nr=0;           /* number of matched pairs remaining in solution */
   int nb=0;           /* number of bad pairs in any iteration */
   int initial_pairs=0;
   int is_ok=0;
...
// :: THIS WAS ADDED TO THE CPP FILE - TO DELETE FROM HERE
/***********************************************************************
...
/* */
   ...
   {
		// larry - free memory if i exist with error as well
