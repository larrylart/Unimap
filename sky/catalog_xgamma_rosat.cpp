...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogXGammaRosat::CSkyCatalogXGammaRosat( CSkyCatalogXGamma* pCatalogXGamma, CUnimapWorker* pWorker ) 
{
...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogXGammaRosat::~CSkyCatalogXGammaRosat()
{
									  ...
									  int bRegion, double nCenterRa, double nCenterDec, double nRadius )
{
	...
	} else if( nLoadType == CATALOG_LOAD_REMOTE )
	{
		if( nLoadSource == CATALOG_QUERY_SOURCE_VIZIER )
		{
			...
			nObj = oRemote.GetXGammaFromVizier( CATALOG_ID_ROSAT, bRegion, nCenterRa, nCenterDec, nRadius, nRadius );
		}
	}
...
// Method:	LoadBinary
// Class:	CSkyCatalogXGammaRosat
// Purpose:	load the binary version of the ROSAT radio sources catalog
// Input:	if to load region or all
// Output:	nothing
/////////////////////////////////////////////////////////////////////
unsigned long CSkyCatalogXGammaRosat::LoadBinary( const wxString& strFile, double nCenterRa, double nCenterDec, 
										double nRadius, int bRegion )
{
	...
	while( !feof( pFile ) )
	{
		...
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].count), sizeof(double), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].bg_count), sizeof(double), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].exp_time), sizeof(double), 1, pFile ) )
			break;
		// :: object/catalog types
//		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].cat_type), sizeof(unsigned char), 1, pFile ) )
//			break;
//		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].type), sizeof(unsigned char), 1, pFile ) )
//			break;
	...
	if( DEBUG_SHOW_PROC_TIMES_ON_FILE_LOAD )
	{
		...
		if( m_pUnimapWorker )
		{
			strLog.Printf( wxT("DEBUG :: Radio %s catalog(%d objects) was loaded in %lu.%lu seconds"), m_pCatalogXGamma->m_strName, 
...
// Method:	ExportBinary
// Class:	CSkyCatalogXGammaRosat
// Purpose:	export ROSAT radio catalog as binary
// Input:	reference vector,size, and file to export to
// Output:	status
/////////////////////////////////////////////////////////////////////
int CSkyCatalogXGammaRosat::ExportBinary( DefCatBasicXGamma* vectCatalog,
									unsigned int nSize )
{
	FILE* pFile = NULL;
	unsigned long i = 0;
	...
	for( i=0; i<nSize; i++ )
	{
		...
		if( !fwrite( (void*) &(vectCatalog[i].dec), sizeof(double), 1, pFile ) )
			break;
		// :: radio sources properties
		if( !fwrite( (void*) &(vectCatalog[i].count), sizeof(double), 1, pFile ) )
			break;
		if( !fwrite( (void*) &(vectCatalog[i].bg_count), sizeof(double), 1, pFile ) )
			break;
		if( !fwrite( (void*) &(vectCatalog[i].exp_time), sizeof(double), 1, pFile ) )
			break;
