 ...
 */
void riset_cir( Now *np, Obj *op, double dis, RiseSet *rp )
{
 ...
 */
void twilight_cir( Now *np, double dis, double *dawn, double *dusk, int *status )
{
	RiseSet rs;
	Obj o;
...
#define	DEGSTEP	5		/* time step is about this many degrees */
	...
	if( Sol_CalcObjOrbitPos( np, op ) < 0 )
	{
	    rp->rs_flags |= RS_ERROR;
	    return;
	}
...
#define	MAXSTEP		(1.0/24.0)/* max time step,days (to detect flat)*/
	...
	do {
	    ...
	    mjd += dt;
	    if( Sol_CalcObjOrbitPos( np, op ) < 0 ) return (-1);
	    a1 = op->s_alt;
