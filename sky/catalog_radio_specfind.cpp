...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogRadioSpecfind::CSkyCatalogRadioSpecfind( CSkyCatalogRadio* pCatalogRadio, CUnimapWorker* pWorker ) 
{
...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogRadioSpecfind::~CSkyCatalogRadioSpecfind()
{
									  ...
									  int bRegion, double nCenterRa, double nCenterDec, double nRadius )
{
	...
	} else if( nLoadType == CATALOG_LOAD_REMOTE )
	{
		if( nLoadSource == CATALOG_QUERY_SOURCE_VIZIER )
		{
			...
			nObj = oRemote.GetRadioFromVizier( CATALOG_ID_SPECFIND, bRegion, nCenterRa, nCenterDec, nRadius, nRadius );
		}
	}
...
/////////////////////////////////////////////////////////////////////
// Method:	CSkyCatalogRadio
// Class:	CSkyCatalogRadioSpecfind
// Purpose:	load the binary version of the PGC galaxy catalog
// Input:	if to load region or all
// Output:	nothing
/////////////////////////////////////////////////////////////////////
unsigned long CSkyCatalogRadioSpecfind::LoadBinary( const wxString& strFile, double nCenterRa, double nCenterDec, 
										double nRadius, int bRegion )
{
	...
	} else
	{
		...
		int nRecords = (int) ( ftell( pFile )/( 23+sizeof(unsigned int)+sizeof(unsigned long)+7*sizeof(double) ));
		// allocate my vector + 1 unit just to be sure
	...
	while( !feof( pFile ) )
	{
		...
		// :: name - 22 chars
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].cat_name), sizeof(char), 23, pFile ) )
			break;
		// :: catalog number 
		...
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].no_spec_points), sizeof(unsigned int), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].frequency), sizeof(double), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].flux_density), sizeof(double), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].i_flux_density), sizeof(double), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].spec_abscissa), sizeof(double), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].spec_slope), sizeof(double), 1, pFile ) )
			break;
...
//			break;
	...
	if( DEBUG_SHOW_PROC_TIMES_ON_FILE_LOAD )
	{
		...
		if( m_pUnimapWorker )
		{
			strLog.Printf( wxT("DEBUG :: DSO %s catalog(%d objects) was loaded in %lu.%lu seconds"), m_pCatalogRadio->m_strName, 
...
// Output:	status
/////////////////////////////////////////////////////////////////////
int CSkyCatalogRadioSpecfind::ExportBinary( DefCatBasicRadio* vectCatalog,
									unsigned int nSize )
{
	FILE* pFile = NULL;
	unsigned long i = 0;
