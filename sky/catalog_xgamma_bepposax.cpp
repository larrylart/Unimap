...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogXGammaBepposax::CSkyCatalogXGammaBepposax( CSkyCatalogXGamma* pCatalogXGamma, CUnimapWorker* pWorker ) 
{
...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogXGammaBepposax::~CSkyCatalogXGammaBepposax()
{
									  ...
									  int bRegion, double nCenterRa, double nCenterDec, double nRadius )
{
	...
	} else if( nLoadType == CATALOG_LOAD_REMOTE )
	{
		if( nLoadSource == CATALOG_QUERY_SOURCE_VIZIER )
		{
			...
			nObj = oRemote.GetXGammaFromVizier( CATALOG_ID_BEPPOSAX_GRBM, bRegion, nCenterRa, nCenterDec, nRadius, nRadius );
		}
	}
...
// Method:	LoadBinary
// Class:	CSkyCatalogXGammaBepposax
// Purpose:	load the binary version of the BEPPOSAX sources catalog
// Input:	if to load region or all
// Output:	nothing
/////////////////////////////////////////////////////////////////////
unsigned long CSkyCatalogXGammaBepposax::LoadBinary( const wxString& strFile, double nCenterRa, double nCenterDec, 
										double nRadius, int bRegion )
{
	...
	while( !feof( pFile ) )
	{
		...
		// :: name - 22 chars
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].cat_name), sizeof(char), 8, pFile ) )
			break;
		// :: catalog number 
		...
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].start_time), sizeof(unsigned long), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].time_det), sizeof(unsigned long), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].interval_no), sizeof(unsigned int), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].duration), sizeof(unsigned long), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].fluence), sizeof(double), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].peak_flux), sizeof(double), 1, pFile ) )
			break;
		if( !fread( (void*) &(m_pCatalogXGamma->m_vectData[m_pCatalogXGamma->m_nData].gamma), sizeof(double), 1, pFile ) )
			break;
...
// Method:	ExportBinary
// Class:	CSkyCatalogXGammaBepposax
// Purpose:	export BEPPOSAX catalog as binary
// Input:	reference vector,size, and file to export to
// Output:	status
/////////////////////////////////////////////////////////////////////
int CSkyCatalogXGammaBepposax::ExportBinary( DefCatBasicXGamma* vectCatalog,
									unsigned int nSize )
{
	FILE* pFile = NULL;
	unsigned long i = 0;
	...
	{
		// :: name - 22 chars
		if( !fwrite( (void*) &(vectCatalog[i].cat_name), sizeof(char), 8, pFile ) )
			break;
		// :: catalog number
		...
		if( !fwrite( (void*) &(vectCatalog[i].dec), sizeof(double), 1, pFile ) )
			break;
		// :: gamma sources properties
		if( !fwrite( (void*) &(vectCatalog[i].start_time), sizeof(unsigned long), 1, pFile ) )
			break;
		if( !fwrite( (void*) &(vectCatalog[i].time_det), sizeof(unsigned long), 1, pFile ) )
			break;
		if( !fwrite( (void*) &(vectCatalog[i].interval_no), sizeof(unsigned int), 1, pFile ) )
			break;
		if( !fwrite( (void*) &(vectCatalog[i].duration), sizeof(unsigned long), 1, pFile ) )
			break;
		if( !fwrite( (void*) &(vectCatalog[i].fluence), sizeof(double), 1, pFile ) )
			break;
		if( !fwrite( (void*) &(vectCatalog[i].peak_flux), sizeof(double), 1, pFile ) )
			break;
		if( !fwrite( (void*) &(vectCatalog[i].gamma), sizeof(double), 1, pFile ) )
			break;
	}
