...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogRadioVlss::CSkyCatalogRadioVlss( CSkyCatalogRadio* pCatalogRadio, CUnimapWorker* pWorker ) 
{
...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogRadioVlss::~CSkyCatalogRadioVlss()
{
...
/////////////////////////////////////////////////////////////////////
unsigned long CSkyCatalogRadioVlss::Load( int nLoadType, int nLoadSource, int nLoadLimit, 
									  int bRegion, double nCenterRa, double nCenterDec, double nRadius )
{
	// check catalog loaded
	if( m_pCatalogRadio->m_vectData != NULL )
	{
		if( m_pCatalogRadio->m_nCatalogLoaded != CATALOG_ID_VLSS )
	...
	} else if( nLoadType == CATALOG_LOAD_REMOTE )
	{
		if( nLoadSource == CATALOG_QUERY_SOURCE_VIZIER )
		{
			...
			nObj = oRemote.GetRadioFromVizier( CATALOG_ID_VLSS, bRegion, nCenterRa, nCenterDec, nRadius, nRadius );
		}
	}
...
// Method:	LoadBinary
// Class:	CSkyCatalogRadioVlss
// Purpose:	load the binary version of the VLSS catalog
// Input:	if to load region or all
// Output:	nothing
/////////////////////////////////////////////////////////////////////
unsigned long CSkyCatalogRadioVlss::LoadBinary( const wxString& strFile, double nCenterRa, double nCenterDec, 
										double nRadius, int bRegion )
{
	...
	} else
	{
		...
		int nRecords = (int) ( ftell( pFile )/( 12+sizeof(unsigned long)+3*sizeof(double) ));
		// allocate my vector + 1 unit just to be sure
	...
	while( !feof( pFile ) )
	{
		...
		m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].frequency = 74.0;

		// :: name - 22 chars
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].cat_name), sizeof(char), 12, pFile ) )
			break;
		// :: catalog number 
...
// Output:	status
/////////////////////////////////////////////////////////////////////
int CSkyCatalogRadioVlss::ExportBinary( DefCatBasicRadio* vectCatalog,
									unsigned int nSize )
{
	FILE* pFile = NULL;
	unsigned long i = 0;
	...
	{
		// :: name - 22 chars
		if( !fwrite( (void*) &(vectCatalog[i].cat_name), sizeof(char), 12, pFile ) )
			break;
		// :: catalog number
