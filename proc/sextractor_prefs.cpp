////////////////////////////////////////////////////////////////////
// Author:		E.BERTIN (IAP)
// Purpose:		Functions to handle the configuration file.
//
// Created:		12/01/2006
// Updated:		10/05/2008
////////////////////////////////////////////////////////////////////
...
// Method:		DumpPrefs
// Class:		CSextractor
// Purpose:		Print the default preference parameters.
// Input:		state
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::DumpPrefs( int state )
{
	char **dp;
...
// Method:		ReadPrefs
// Class:		CSextractor
// Purpose:		Read a configuration file in ``standard'' format 
//				(see the SExtractor documentation)
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ReadPrefs( char *filename, char **argkey, 
											char **argval, int narg )
{
	...
	for (i=0; key[i].name[0]; i++) strcpy(keylist[i], key[i].name);
	keylist[i][0] = '\0';
	...
	for (warn=0;;)
	{
		...
		if (!flagc && !flagd)
			if (flage || !fgets(str, MAXCHARL, infile)) flagc=1;
		...
		{
			if (warn>=10) error(EXIT_FAILURE, "*Error*: No valid keyword found in ", filename);
			nkey = FindKeysOrig(keyword, keylist, FIND_STRICT);
			...
			if (nkey!=RETURN_ERROR)
			{
				...
				if (value && (dolpos=strchr(value, '$')))
				{
					...
					while (dolpos)
					{
						...
						if (*(++valuet) == (char)'{') valuet++;

						strncpy(envname, valuet, nc=strcspn(valuet,"}/:\"\'\\"));
						*(envname+nc) = (char)'\0';
						if (*(valuet+=nc) == (char)'}') valuet++;

						if (!(envval=getenv(envname))) 
							error(EXIT_FAILURE, "Environment variable not found: ", envname);

						// Copy the ENV content 
						while(*envval)			
							*(value2t++) = *(envval++);

						// Continue verbatim copy 
						while(*valuet && *valuet!=(char)'$')
							*(value2t++) = *(valuet++);
				...
				switch(key[nkey].type)
				{
					...
					case P_BOOL:
					{
						...
						if ((cp = (char*) strchr("yYnN", (int)value[0])))
					...
					default:
					{
						error(EXIT_FAILURE, "*Internal ERROR*: Type Unknown", " in readprefs()");
						break;
					}
				}
...
// Method:		ClosePrefs
// Class:		CSextractor
// Purpose:		implemented by larry to remove memory allocated
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ClosePrefs( )
{
	int i =0, j=0;

//	int nkey = findkeys(keyword, keylist, FIND_STRICT);
	for (i=0; key[i].name[0]; i++)
	{
		if( key[i].type == P_STRINGLIST )
		{
			for (j=0; j<*(key[i].nlistptr); j++)
			{
				if( ((char **)key[i].ptr)[j] )
				{
					free(((char **)key[i].ptr)[j]);
					((char **)key[i].ptr)[j] =  NULL;
				}
			}
		}
	}
...
// Method:		FindKeysOrig
// Class:		CSextractor
// Purpose:		find an item within a list of keywords, SExtractor 
//				version.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
int CSextractor::FindKeysOrig( char *str, char** keyw, int mode )
{
	int i;
...
// Output:		nothing
////////////////////////////////////////////////////////////////////
int CSextractor::FindKeys( char *str, char keyw[][32], int mode )
{
	int i;
...
// Method:		CiStrCmp
// Class:		CSextractor
// Purpose:		case-insensitive strcmp.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
int CSextractor::CiStrCmp( char *cs, char *ct, int mode )
{
   int  i, diff;
...
// Method:		UsePrefs
// Class:		CSextractor
// Purpose:		Update various structures according to the prefs.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::UsePrefs( )
{
