...
//#include	"sextractor_param.h"
//#include	"sextractor_sexhead.h"
//#include	"sextractor_sexhead1.h"
//#include "fits/fitscat.h"
#include	"sextractor_sexheadsc.h"
...
////////////////////////////////////////////////////////////////////
// Method:		ReadCatParams
// Class:		CSextractor
// Purpose:		Read the catalog config file
// Input:		file name with params
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ReadCatParams( char *filename )
{
	keystruct	*key;
	FILE		*infile;
	...
	while( fgets(str, MAXCHAR, infile) )
	{
		...
		{
			keyword = strtok(sstr, " \t{[(\n\r");
			if( keyword && (i = findkey(keyword,(char *)objkey,sizeof(keystruct)))!=RETURN_ERROR )
			{
				key = objkey+i;
				...
				*((char *)key->ptr) = (char)'\1';
				...
				if( key->naxis ) 
				{
					...
					{
						if( i>=key->naxis )
							error( EXIT_FAILURE, "*Error*: too many dimensions for keyword ", keyword );
						if( !(size*=(key->naxisn[i]=atoi(sstr))) )
							error( EXIT_FAILURE, "*Error*: wrong array syntax for keyword ", keyword );
					}
					key->nbytes = size;
				}
			} else
...
////////////////////////////////////////////////////////////////////
// Method:		ReadCatParamsHardcode
// Class:		CSextractor
// Purpose:		Read the catalog config from a hardcode string
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ReadCatParamsHardcode(  )
{
	keystruct	*key;
	FILE		*infile;
	char		str[MAXCHAR], *keyword, *sstr;
	...
	thecat.nparam = 0;
//	while( fgets(str, MAXCHAR, infile) )
	for( n=0; n<m_nCatParams; n++ )
	{
		// make a copy from vector
		strcpy( str, m_vectCatParams[n] );
		// ... and continue from here with normal processing
		sstr = str + strspn(str," \t");
		if (*sstr!=(char)'#' && *sstr!=(char)'\n')
		{
			keyword = strtok(sstr, " \t{[(\n\r");
...
////////////////////////////////////////////////////////////////////
// Method:		UpdateParamFlags
// Class:		CSextractor
// Purpose:		Update parameter flags according to their mutual 
//				dependencies.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::UpdateParamFlags( )
{
   int	i;
...
// Method:		InitCat
// Class:		CSextractor
// Purpose:		Initialize the catalog header
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::InitCat( void )
{
	keystruct* key=NULL;
	int	i=0, n=0;

	if( prefs.cat_type == CAT_NONE ) return;

	update_tab(objtab);

	// :: IF TYPE ASCII/VOTABLE
		...
		prefs.cat_type == ASCII_SKYCAT || prefs.cat_type == ASCII_VO )
	{
		...
		{
			if (objtab->nkey<3) error(EXIT_FAILURE,"The SkyCat format requires at least 4 parameters:", " Id Ra Dec Mag");
			...
			{
				if( i>4 ) fprintf( ascfile, "\t%s", key->name );
		...
		{
			WriteXmlHeader(ascfile);
			WriteVoFields(ascfile);
	...
	} else
	{
		...
		if( open_cat(fitscat, WRITE_ONLY) != RETURN_OK ) error( EXIT_FAILURE,"*Error*: cannot open for writing ",prefs.cat_name );
		...
		switch(prefs.cat_type)
		{
			...
			default:
				error( EXIT_FAILURE, "*Internal Error*: Unknown FITS type in ", "initcat()");
		}
	}
...
////////////////////////////////////////////////////////////////////
// Method:		WriteVoFields
// Class:		CSextractor
// Purpose:		Write the list of columns to an XML-VOTable file or 
//				stream
// Input:		Pointer to the output file (or stream).
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::WriteVoFields( FILE *file )
{
   keystruct	*key;
...
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ReInitCat( picstruct *field )
{
...
/*------ We create a dummy table (only used through its header) */
        ...
        key->ttype = T_STRING;
        key->nobj = 1;
        key->nbytes =  80*(fitsfind((char*)key->ptr, "END     ")+1);
        key->naxis = 2;
...
/*------ We create a dummy table (only used through its header) */
        ...
        key->htype = H_STRING;
        key->ttype = T_STRING;
        key->nobj =  fitsfind((char*)key->ptr, "END     ")+1;
        key->nbytes = 80;
        key->naxis = 1;
        QMALLOC(key->naxisn, int, key->naxis);
        key->naxisn[0] = 80;
...
// Method:		WriteCat
// Class:		CSextractor
// Purpose:		Write out in the catalog each one object.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::WriteCat( int n, objliststruct *objlist )
{
...
// Method:		EndCat
// Class:		CSextractor
// Purpose:		Terminate the catalog output.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::EndCat( char *error )
{
   keystruct	*key;
   int		i;
	...
	if (!catopen_flag)
	{
		if (prefs.cat_type == ASCII_VO) WriteXmlError(prefs.cat_name, error);
		/* Free allocated memory for arrays */
		key = objtab->key;
		for (i=objtab->nkey; i--; key=key->nextkey)
			if (key->naxis && key->allocflag) free(key->ptr);
...
void CSextractor::ReEndCat( )
{
  ...
  switch(prefs.cat_type)
	...
	|| !(key=name_to_key(tab, "Field Header Card")))
      ...
      fitswrite(head, "SEXDATE ", thecat.ext_date, H_STRING, (t_type) 0);
      fitswrite(head, "SEXTIME ", thecat.ext_time, H_STRING, (t_type) 0);
