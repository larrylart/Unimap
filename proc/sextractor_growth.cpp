...
// Method:		InitGrowth
// Class:		CSextractor
// Purpose:		Allocate memory for growth curve stuff.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::InitGrowth( )
{
...
// Method:		EndGrowth
// Class:		CSextractor
// Purpose:		Free memory occupied by growth curve stuff.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::EndGrowth( )
{
	obj2struct	*obj2 = &outobj2;

	free(growth);	
	if( FLAG(obj2.flux_radius) && !prefs.flux_radiussize ) free(obj2->flux_radius);

	return;
}

////////////////////////////////////////////////////////////////////
// Method:		MakeAverGrowth
// Class:		CSextractor
// Purpose:		Build growth curve based on averages.
// Input:		picture, w ? and object strcuture
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::MakeAverGrowth( picstruct *field, picstruct *wfield, 
													objstruct *obj )
{
	obj2struct	*obj2 = &outobj2;
	...
	double		*growtht,
				...
				offsetx,offsety,scalex,scaley,scale2, ngamma, locarea=0.0,
	...
	LONG		pos;
	PIXTYPE		*strip,*stript, *wstrip, *wstript, pdbkg, wthresh;
	...
	if( rlim<prefs.autoaper[0] ) rlim = prefs.autoaper[0];
	...
	for( y=ymin; y<ymax; y++ )
	{
		...
		for( x=xmin; x<xmax; x++, stript++, wstript++ )
		{
			...
			if( (r2=dx*dx+dy*dy) < rextlim2 )
			{
				...
				if( pflag ) pix = exp(pix/ngamma) - pdbkg;
...
/*
	...
	{
		tv -= area*obj->dbkg;
		if( !wfield && gain > 0.0 && tv>0.0 ) sigtv += tv/gain;
	}
*/
	...
	if( FLAG(obj2.flux_radius) )
	{
		...
		for( j=0; j<prefs.nflux_frac; j++ )
		{
			...
			obj2->flux_radius[j] = step *(i? ((dg=*growtht - *(growtht-1)) != 0.0 ? i + (tv - *(growtht-1))/dg : i) : (*growth !=0.0 ?tv/(*growth) : 0.0));
		}
	}
	...
	if( FLAG(obj2.hl_radius) )
	{
		...
		growtht = growth-1;
		for (i=0; i<n && *(++growtht)<tv; i++);
		obj2->hl_radius = step*(i? ((dg=*growtht - *(growtht-1)) != 0.0 ? i + (tv - *(growtht-1))/dg : i) : (*growth !=0.0 ?tv/(*growth) : 0.0));
	}
