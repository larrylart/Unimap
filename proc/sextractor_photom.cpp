...
////////////////////////////////////////////////////////////////////
// Method:		ComputeAperFlux
// Class:		CSextractor
// Purpose:		Compute the total flux within a circular aperture.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void  CSextractor::ComputeAperFlux( picstruct *field, picstruct *wfield, objstruct *obj, int i )
{
	float r2, raper,raper2, rintlim,rintlim2,rextlim2, mx,my,dx,dx1,dy,dy2,
	...
	if( wfield ) wthresh = wfield->weight_thresh;
	wstrip = wstript = NULL;
	mx = obj->mx;
	my = obj->my;
	w = field->width;
	...
	strip = field->strip;
	if (wfield) wstrip = wfield->strip;

	for (y=ymin; y<ymax; y++)
	{
		stript = strip + (pos = (y%h)*w + xmin);
		if (wfield) wstript = wstrip + pos;
		for (x=xmin; x<xmax; x++, stript++, wstript++)
		{
			...
			if ((r2=dx*dx+dy*dy) < rextlim2)
			{
				if (r2> rintlim2)
				{
					...
					for (sy=APER_OVERSAMP; sy--; dy+=scaley)
					{
						...
						for (sx=APER_OVERSAMP; sx--; dx1+=scalex)
							if (dx1*dx1+dy2<raper2) locarea += scale2;
					}
				...
				{
					if( corrflag && (x2=(int)(2*mx+0.49999-x))>=0 && x2<w
				...
				tv += locarea*pix;
				if( gainflag && pix>0.0 && gain>0.0 ) sigtv += pix/gain*var/backnoise2;
			}
		}
	}
	...
	if( i<prefs.flux_apersize ) obj2->flux_aper[i] = tv;
	if( i<prefs.fluxerr_apersize ) obj2->fluxerr_aper[i] = sqrt(sigtv);
	if( i<prefs.mag_apersize ) obj2->mag_aper[i] = tv>0.0 ? -2.5*log10(tv) + prefs.mag_zeropoint : 99.0;
	if( i<prefs.magerr_apersize ) obj2->magerr_aper[i] = tv>0.0 ? 1.086*sqrt(sigtv)/tv:99.0;

	return;
}

////////////////////////////////////////////////////////////////////
// Method:		ComputePetroFlux
// Class:		CSextractor
// Purpose:		Compute the total flux within an automatic elliptical
//				aperture.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void  CSextractor::ComputePetroFlux( picstruct *field, picstruct *dfield, 
									picstruct *wfield, picstruct *dwfield, 
									objstruct *obj )
{
	double sigtv, tv, r1, v1,var,gain,backnoise2, muden,munum;
	float bkg, ngamma, mx,my, dx,dy, cx2,cy2,cxy, r2, klim, klim2,kmin,kmin2,
			kmax,kmax2,kstep,kmea,kmea2, dxlim, dylim;
	int area=0, areab=0, areaden=0, areanum=0, x, y, x2, y2, xmin, xmax, ymin, ymax, fymin, fymax, w, h,
			pflag, corrflag, gainflag, pos;
	PIXTYPE *strip, *stript, *dstrip, *dstript, *wstrip, *wstript, *dwstrip, *dwstript,
	...
	if( dwfield ) dwthresh = dwfield->weight_thresh;
	wstrip = dwstrip = NULL;
	if( wfield ) wthresh = wfield->weight_thresh;
	...
	dstrip = dfield->strip;
	if( dwfield ) dwstrip = dwfield->strip;
	klim = sqrt(klim2);
	kstep = klim/20.0;
	...
	for( kmin=kstep; (kmax=kmin*1.2)<klim; kmin += kstep )
	{
		...
		for (y=ymin; y<ymax; y++)
		{
			...
			if (dwfield) dwstript = dwstrip + pos;
	...
	if (area)
	{
		...
		if( areanum && areaden && munum && muden )
		{
			...
			if (obj2->petrofactor < prefs.petroparam[1]) obj2->petrofactor = prefs.petroparam[1];

		} else
			obj2->petrofactor = prefs.petroparam[1];

		// Flag if the Petrosian photometry can be strongly affected by neighhours
		if( (float)areab/area > CROWD_THRESHOLD ) obj->flag |= OBJ_CROWDED;
		...
		// Flag if the Petrosian photometry can be strongly affected by neighhours 
		if( (float)areab > CROWD_THRESHOLD*area ) obj->flag |= OBJ_CROWDED;
	...
	if( FLAG(obj2.mag_petro) )
		obj2->mag_petro = obj2->flux_petro>0.0 ? -2.5*log10(obj2->flux_petro) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_petro) )
		obj2->magerr_petro = obj2->flux_petro>0.0 ? 1.086*obj2->fluxerr_petro/obj2->flux_petro:99.0;
	if( tv<=0.0 ) obj2->petrofactor = 0.0;

	return;
}

////////////////////////////////////////////////////////////////////
// Method:		ComputeAutoFlux
// Class:		CSextractor
// Purpose:		Compute the total flux within an automatic elliptical
//				aperture.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ComputeAutoFlux(picstruct *field, picstruct *dfield, 
								  picstruct *wfield, picstruct *dwfield, 
									objstruct *obj)
{
	double sigtv, tv, r1, v1,var,gain,backnoise2;
	float bkg, ngamma, mx,my, dx,dy, cx2,cy2,cxy, r2,klim2, dxlim, dylim;
	int area,areab, x,y, x2,y2, xmin,xmax,ymin,ymax, fymin,fymax, w,h,
	pflag, corrflag, gainflag, pos;
	...
	*dwstrip,*dwstript,
	...
	if (area)
	{
		...
		if (r1>0.0 && v1>0.0)
		{
			...
			if (obj2->kronfactor < prefs.autoparam[1]) obj2->kronfactor = prefs.autoparam[1];
	...
	if( FLAG(obj2.mag_auto) )
		obj2->mag_auto = obj2->flux_auto>0.0 ? -2.5*log10(obj2->flux_auto) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_auto) )
		obj2->magerr_auto = obj2->flux_auto>0.0 ? 1.086*obj2->fluxerr_auto/obj2->flux_auto:99.0;
	if( tv<=0.0 ) obj2->kronfactor = 0.0;

	return;
}

////////////////////////////////////////////////////////////////////
// Method:		ComputeIsocorFlux
// Class:		CSextractor
// Purpose:		Compute the (corrected) isophotal flux.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ComputeIsocorFlux( picstruct *field, objstruct *obj )
{
	double	ati;
...
// Method:		ComputeMags
// Class:		CSextractor
// Purpose:		Compute magnitude parameters.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ComputeMags( picstruct *field, objstruct *obj )
{
	obj2struct	*obj2 = &outobj2;

	// Mag. isophotal 
	if( FLAG(obj2.mag_iso) )
		obj2->mag_iso = obj2->flux_iso>0.0 ? -2.5*log10(obj2->flux_iso) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_iso) )
		obj2->magerr_iso = obj2->flux_iso>0.0 ? 1.086*obj2->fluxerr_iso/obj2->flux_iso:99.0;

	// Mag. isophotal corrected
	if( FLAG(obj2.mag_isocor) )
		obj2->mag_isocor = obj2->flux_isocor>0.0 ? -2.5*log10(obj2->flux_isocor) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_isocor) )
		obj2->magerr_isocor = obj2->flux_isocor>0.0 ? 1.086*obj2->fluxerr_isocor/obj2->flux_isocor:99.0;
	...
	if( FLAG(obj2.mag_best) )
		obj2->mag_best = obj2->flux_best>0.0 ? -2.5*log10(obj2->flux_best) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_best) )
		obj2->magerr_best = obj2->flux_best>0.0 ? 1.086*obj2->fluxerr_best/obj2->flux_best:99.0;

	// Mag. SOM-fit 
	if( FLAG(obj2.mag_somfit) )
		obj2->mag_somfit = obj2->flux_somfit>0.0 ? -2.5*log10(obj2->flux_somfit) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_somfit) )
		obj2->magerr_somfit = obj2->flux_somfit>0.0 ? 1.086*obj2->fluxerr_somfit/obj2->flux_somfit:99.0;

	// Mag. PROFILE 
	if( FLAG(obj2.mag_prof) )
		obj2->mag_prof = obj2->flux_prof>0.0 ? -2.5*log10(obj2->flux_prof) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_prof) )
		obj2->magerr_prof = obj2->flux_prof>0.0 ? 1.086*obj2->fluxerr_prof/obj2->flux_prof:99.0;

	// Mag. WINdowed 
	if( FLAG(obj2.mag_win) )
		obj2->mag_win = obj2->flux_win>0.0 ? -2.5*log10(obj2->flux_win) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_win) )
		obj2->magerr_win = obj2->flux_win>0.0 ? 1.086*obj2->fluxerr_win/obj2->flux_win:99.0;
	// Mag. GALFIT 
	if( FLAG(obj2.mag_galfit) )
		obj2->mag_galfit = obj2->flux_galfit>0.0 ? -2.5*log10(obj2->flux_galfit) + prefs.mag_zeropoint:99.0;
	if( FLAG(obj2.magerr_galfit) )
		obj2->magerr_galfit = obj2->flux_galfit>0.0 ? 1.086*obj2->fluxerr_galfit/obj2->flux_galfit:99.0;

	// SB units 
	if( FLAG(obj2.maxmu) )
		outobj2.maxmu = obj->peak > 0.0 ? -2.5*log10((obj->peak) / (field->pixscale * field->pixscale)) + prefs.mag_zeropoint : 99.0;

	if( FLAG(obj2.threshmu) )
		obj2->threshmu = obj->thresh > 0.0 ? -2.5*log10((obj->thresh) / (field->pixscale * field->pixscale)) + prefs.mag_zeropoint : 99.0;
