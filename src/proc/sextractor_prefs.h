...
// larry added for cpp ??
typedef enum	{QUIET, NORM, WARN, FULL} RuntimeVerboseType;
typedef enum	{THRESH_RELATIVE, THRESH_ABSOLUTE} BkgndTreshType;
typedef enum	{GLOBAL, LOCAL} BkgndVarType;
typedef enum	{ASSOCSELEC_ALL, ASSOCSELEC_MATCHED, ASSOCSELEC_NOMATCHED} DefAssocType;
typedef enum	{PSFDISPLAY_SPLIT, PSFDISPLAY_VECTOR} DefPsfDisplayType;
typedef enum	{INTERP_NONE, INTERP_VARONLY, INTERP_ALL} DefInterpolationType;
typedef enum	{CCD, PHOTO} DefPhotoType;
typedef enum	{MASK_NONE, MASK_BLANK, MASK_CORRECT} DefMaskType;
typedef enum	{FLAG_OR, FLAG_AND, FLAG_MIN, FLAG_MAX, FLAG_MOST} DefFlagType;
typedef enum	{ASSOC_FIRST, ASSOC_NEAREST, ASSOC_MEAN, ASSOC_MAGMEAN,
					ASSOC_SUM, ASSOC_MAGSUM, ASSOC_MIN, ASSOC_MAX} DefAssocallType;
typedef enum	{CAT_NONE, ASCII, ASCII_HEAD, ASCII_SKYCAT, ASCII_VO,
					FITS_LDAC, FITS_TPX, FITS_10} DefCatalogType;
//typedef enum	{PNONE, FIXED, AUTO} DepAperType;
typedef enum	{IMAGE, AFILE} DefOriginOfBg;
...
//  enum	{THRESH_RELATIVE, THRESH_ABSOLUTE}
	BkgndTreshType			thresh_type[2];	/* bkgnd type */
...
//	FITS_LDAC, FITS_TPX, FITS_10}
	DefCatalogType		cat_type;				/* type of catalog */

	// larry :: here could a problem with fixed - is already defined in wingdi
...
/*----- background */
//  enum	{IMAGE, AFILE}			
  DefOriginOfBg back_origin;	/* origin of bkgnd */
...
//  enum	{GLOBAL, LOCAL}
	BkgndVarType		pback_type;	/* phot. bkgnd type */
...
//  enum	{QUIET, NORM, WARN, FULL}
	RuntimeVerboseType	verbose_type;	/* display type */
...
//	 ASSOC_SUM, ASSOC_MAGSUM, ASSOC_MIN, ASSOC_MAX}
	DefAssocallType		assoc_type;				/* type of assoc. */
//  enum	{ASSOCSELEC_ALL, ASSOCSELEC_MATCHED, ASSOCSELEC_NOMATCHED}
	DefAssocType		assocselec_type;		       	/* type of assoc. */
...
//  enum {MASK_NONE, MASK_BLANK, MASK_CORRECT}
	DefMaskType				mask_type;				/* type of masking */
...
//  enum	{INTERP_NONE, INTERP_VARONLY, INTERP_ALL}
	DefInterpolationType	interp_type[2];				/* interpolat. type */
...
//  enum	{PSFDISPLAY_SPLIT, PSFDISPLAY_VECTOR}
	DefPsfDisplayType	psfdisplay_type;			/* PSF display type */
