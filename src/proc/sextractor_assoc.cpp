////////////////////////////////////////////////////////////////////
// Purpose:	Routines for catalog-associations.
////////////////////////////////////////////////////////////////////
...
// Method:		CompAssoc
// Class:		Glogal
// Purpose:		Comparison function for sort_assoc().
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
...
// Method:		SortAssoc
// Class:		CSextractor
// Purpose:		Make the presentation histogram, order the assoc-list
//				and build the hash-table.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::SortAssoc( picstruct *field, assocstruct *assoc )
{
//   int		CompAssoc(const void *i1, const void *i2);
   float	*list, rad;
  ...
  nobj = assoc->nobj;
  list = assoc->list;
  qsort(assoc->list, assoc->nobj, step*sizeof(float), CompAssoc );
...
// Method:		LoadAssoc
// Class:		CSextractor
// Purpose:		Read an assoc-list, and returns a pointer to the new 
//				assoc struct (or NULL if no list was found).
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
assocstruct* CSextractor::LoadAssoc( char *filename )
{
	assocstruct *assoc;
	FILE *file;
	float *list, val;
	char str[MAXCHAR], str2[MAXCHAR], *sstr;
	int *data, i, ispoon, j, k, l, ncol, ndata, nlist, size,spoonsize,
	...
	for (i=0; fgets(str, MAXCHAR, file);)
	{
		...
		if( !i )
		{
			...
			if( !ncol ) error(EXIT_FAILURE, "*Error*: empty line in ", filename);
			...
			for( j=0; j<prefs.nassoc_data && k<=prefs.assoc_size; j++ )
				if( (l=prefs.assoc_data[j]) && --l<ncol ) data[l] = k++;
			ndata = k-1;
			if( !ndata )
			{
				ndata = ncol;
				if( prefs.assoc_size<ndata ) ndata = prefs.assoc_size;
				for( j=0; j<ndata; j++ ) data[j] = j+1;
			}
      ...
      if( (xindex = prefs.assoc_param[0]-1) >= ncol ) 
        error(EXIT_FAILURE, "*Error*: ASSOC_PARAMS #1 exceeds the number of ", "fields in the ASSOC file");
      if( (yindex = prefs.assoc_param[1]-1) >= ncol ) 
        error(EXIT_FAILURE, "*Error*: ASSOC_PARAMS #2 exceeds the number of ", "fields in the ASSOC file");
...
// Method:		InitAssoc
// Class:		CSextractor
// Purpose:		Initialize the association procedure.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::InitAssoc( picstruct *field )
{
	assocstruct	*assoc;

	// Load the assoc-list
	if( !(assoc = field->assoc = LoadAssoc(prefs.assoc_name)) )
		error(EXIT_FAILURE, "*Error*: Assoc-list file not found: ", prefs.assoc_name);

	// Sort the assoc-list by y coordinates, and build the hash table 
	SortAssoc(field, assoc);
...
// Method:		EndAssoc
// Class:		CSextractor
// Purpose:		Free memory related to the assoc operations.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::EndAssoc( picstruct *field )
{
	// Free the assoc-list 
	if( field->assoc )
	{
...
// Method:		DoAssoc
// Class:		CSextractor
// Purpose:		Perform the association task for a source and return 
//				the number of IDs.
// Input:		picture structure, x, y ?
// Output:		nothing
////////////////////////////////////////////////////////////////////
int CSextractor::DoAssoc( picstruct *field, float x, float y )
{
	assocstruct	*assoc;
	double aver;
	float dx,dy, dist, rad, rad2, comp, wparam, *list, *input, *data;
	...
	for( flag = 0; (h++<nobj && *(list+1)-rad<y); list+=step )
	{
		...
		if ((dist=dx*dx+dy*dy)<rad2)
		{
			...
			switch(prefs.assoc_type)
			{
				...
				default:
					error(EXIT_FAILURE, "*Internal Error*: unknown ASSOC type in ", "pixlearn()");
			}
		}
	}
