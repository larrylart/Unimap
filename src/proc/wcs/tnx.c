...
tnxaxisstruct	*read_tnxaxis(char *tnxstr)
  ...
  if ((pstr=strpbrk(tnxstr, "1234567890-+.")))
    ...
    if (!(tnxaxis=malloc(sizeof(tnxaxisstruct))))
      return NULL;
    tnxaxis->type = (int)(atof(strtok(pstr, " ", &ptr))+0.5);
    tnxaxis->xorder = (pstr=strtok(NULL, " ", &ptr))?
			(int)(atof(pstr)+0.5) : 0;
    tnxaxis->yorder = (pstr=strtok(NULL, " ", &ptr))?
			(int)(atof(pstr)+0.5) : 0;
    tnxaxis->xterms = (pstr=strtok(NULL, " ", &ptr))?
			(int)(atof(pstr)+0.5) : 0;
    min = (pstr=strtok(NULL, " ", &ptr))? atof(pstr) : 0.0;
    max = (pstr=strtok(NULL, " ", &ptr))? atof(pstr) : 0.0;
    if (max <= min)
      return NULL;
...
/*-- Now read the mapping coefficients */
    if (!(tnxaxis->coeff=malloc(tnxaxis->ncoeff*sizeof(double))))
      return NULL;
    for (i=0; i<tnxaxis->ncoeff && (pstr=strtok(NULL, " ", &ptr)); i++)
