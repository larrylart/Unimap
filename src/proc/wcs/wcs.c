...
*   unknown elements.
*
*   An initialization routine, rwcsset(), computes indices from the ctype
*   array but need not be called explicitly - see the explanation of
*   wcs.flag below.
*
*
*   Initialization routine; rwcsset()
*   --------------------------------
...
*             0 to 5 as above.
*
*       These routines support both methods; rwcsset() determines which is
...
*         rwcsset() to redetermine the indices.  The flag is set to 999 if
...

{
   ...
   if (wcs->flag != WCSSET) {
      if (rwcsset(lin->naxis, ctype, wcs)) return 1;
   }
...
/*--------------------------------------------------------------------------*/
// larry added an r in front
int rwcsrev(ctype, wcs, pixcrd, lin, imgcrd, prj, phi, theta, crval, cel,
    world)
