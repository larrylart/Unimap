...
// Purpose:		Load a new strip of pixel data into the buffer.
// Input:		picstruct *field, picstruct *wfield, unsigned long nImgPointer
// Output:		void pointer to the strip read
////////////////////////////////////////////////////////////////////
void* CSextractor::LoadImgStrip( picstruct *field, picstruct *wfield )
{
	checkstruct	*check;
	...
	if( !field->y )
	{
		...
		if (flags ^ FLAG_FIELD)
		{
			...
			// larry ::: here data in fact is field->strip :)
			data = field->strip;

			// We assume weight data have been read just before 
			if (interpflag) wdata = wfield->strip;

			if (flags & BACKRMS_FIELD)
			{
				// Bicubic-spline interpolation of the background noise 
				// along the current scanline (y).
			...
			for (y=0; y<field->stripheight; y++, data += w)
			{
				...
				if (prefs.check_flag)
				{
					...
					if( (flags&DETECT_FIELD) && (check=prefs.check[CHECK_BACKRMS]) )
					{
						...
						WriteCheck( check, (float*) check->pix, w );
					}
				}
			}
	...
	} else
    {
		...
		if (flags ^ FLAG_FIELD)
		{
			...
			// We assume weight data have been read just before 
			if( interpflag ) wdata = wfield->strip + field->stripylim*w;
			...
			{
				// larry added here new method
				//readdata(field, data, w);
				ReadAstroImageData(field, data, w );
			}

			if (flags & (WEIGHT_FIELD|RMS_FIELD|BACKRMS_FIELD|VAR_FIELD))
				WeightToVar(field, data, w);

			if ((flags & MEASURE_FIELD) && (check=prefs.check[CHECK_IDENTICAL]))
				WriteCheck(check, data, w);

			// Interpolate and subtract the background at current line 
			if (flags & (MEASURE_FIELD|DETECT_FIELD)) SubBackLine(field, field->ymax, data);
			if (interpflag) Interpolate(field,wfield, data, wdata);
...
// Output:		nothing
////////////////////////////////////////////////////////////////////
void* CSextractor::LoadStrip( picstruct *field, picstruct *wfield )
{
	checkstruct	*check;
	int		y, w, flags, interpflag;
	PIXTYPE	*data, *wdata, *rmsdata;
	...
	if( !field->y )
	{
		...
		if (flags ^ FLAG_FIELD)
		{
			...
			{
				// This is the only place where one can pick-up safely the current bkg
				if (flags & (MEASURE_FIELD|DETECT_FIELD)) SubBackLine(field, y, data);
...
// Method:		CopyData
// Class:		CSextractor
// Purpose:		Copy image data from one field to the other.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::CopyData( picstruct *field, int offset, int size )
{
...
////////////////////////////////////////////////////////////////////
// Method:		ReadAstroImageData
// Class:		GLOBAL::Sextractor
// Purpose:		read image data from opencv bufer in PIXTYPE (float) 
//				format buffer 
// Input:		picstruct *field, PIXTYPE *ptr, int size
// Output:		last step
////////////////////////////////////////////////////////////////////
void CSextractor::ReadAstroImageData( picstruct *field, PIXTYPE *ptr, int size )
{
	PIXTYPE	bs,bz;
	int		i, bowl, spoonful, npix, curval, dval;
	unsigned char nBlue, nGreen, nRed;

	unsigned long nStep = m_nGlobImgPointer;

	// fits scale and zero point ?
	...
	{
		if (spoonful>size) spoonful = size;
		for (i=spoonful; i--;)
		{
			// copy and convert from opencv data 
//			nBlue = (unsigned char) field->opencvbuff[nStep];nStep++;
//			nGreen = (unsigned char) field->opencvbuff[nStep];nStep++;
//			nRed = (unsigned char) field->opencvbuff[nStep];nStep++;
//			*(ptr++) = (double) 0.299*nRed + 0.587*nGreen + 0.114*nBlue;

			*(ptr++) = (float) field->vectImageBuff[nStep];
			nStep++;
		}

	}

	m_nGlobImgPointer = nStep;
...
// Method:		ReadData
// Class:		CSextractor
// Purpose:		read and convert input data stream in PIXTYPE (float) 
//				format.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ReadData( picstruct *field, PIXTYPE *ptr, int size )
{
		...
		case ICOMPRESS_NONE:
		{
			...
			for(; size>0; size -= spoonful)
			{
				...
				switch(field->bitpix)
				{
					...
					case BP_LONG:
					{
						if (bswapflag) swapbytes(bufdata, 4, spoonful);
						if (field->bitsgn)
						{
					...
					case BP_DOUBLE:
					{
						if (bswapflag) swapbytes(bufdata, 8, spoonful);
		...
		while (size--)
		{
			if (!(npix--))
			{
				if (curval != field->compress_checkval)
					error(EXIT_FAILURE, "*Error*: invalid BASEBYTE checksum in ", field->filename);

				bufdata = field->compress_buf;
				QFREAD(bufdata, FBSIZE, field->file, field->filename);
				curval = 0;

				if (bswapflag) swapbytes( bufdata, 4, 1 );
				field->compress_checkval = *((int *)bufdata);
				bufdata += 4;

				if (bswapflag) swapbytes(bufdata, 2, 1);
...
// Method:		ReadIData
// Class:		CSextractor
// Purpose:		read and convert input data stream in FLAGTYPE 
//				(unsigned int) format.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ReadIData( picstruct *field, FLAGTYPE *ptr, int size )
{
  static char	bufdata0[DATA_BUFSIZE];
  char		*bufdata;
  short		val16;
...
////////////////////////////////////////////////////////////////////
// Method:		ReadImageHead
// Class:		CSextractor
// Purpose:		extract some data from the FITS-file header
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ReadImageHead( picstruct *field )
{
   int		j,l, n;
...
/*-- Test if the WCS is recognized and a celestial pair is found */
    if (prefs.world_flag
	&& !l_wcsset(as->naxis,(const char(*)[9])as->ctype, as->wcs)
	&& as->wcs->flag<999)
      {
       char	*pstr;
       double	date;
...
// Method:		ReadFitsHead
// Class:		CSextractor
// Purpose:		read data from the FITS-file header
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
char* CSextractor::ReadFitsHead( FILE *file, char *filename, int *nblock )
{
	int     n;
	char    *buf;
	...
	if (strncmp(buf, "SIMPLE  ", 8))
	{
		...
		{
			memset( buf, ' ', 80 );
			strncpy( buf, "SIMPLE  =                    T / Decompressed ", 59);
		}
		*/
	}
