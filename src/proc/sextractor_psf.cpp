...
// Method:		PsfInit
// Class:		CSextractor
// Purpose:		Allocate memory and stuff for the PSF-fitting.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::PsfInit( psfstruct *psf )
{
...
// Method:		PsfEnd
// Class:		CSextractor
// Purpose:		Free memory occupied by the PSF-fitting stuff.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::PsfEnd( psfstruct *psf, psfitstruct *psfit )
{
	int	d, ndim;

	if (psf->pc) PcEnd(psf->pc);
...
// Method:		PsfLoad
// Class:		CSextractor
// Purpose:		Read the PSF data from a FITS file.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
psfstruct* CSextractor::PsfLoad( char *filename )
{
...
/*---- Polynomial degree for each group */
    ...
    psf->poly = PolyInit(group, ndim, deg, ngroup);
...
/*-- This is a simple, constant PSF */
    psf->poly = PolyInit(group, 0, deg, 0);
    psf->context = NULL;
    }
...
////////////////////////////////////////////////////////////////////
// Method:		PsfReadContext
// Class:		CSextractor
// Purpose:		Read the PSF context parameters in the FITS header.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::PsfReadContext( psfstruct *psf, picstruct *field )
{
...
// Method:		PsfFit
// Class:		CSextractor
// Purpose:		standart PSF fit for one component  
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::PsfFit( psfstruct *psf, picstruct *field, 
							picstruct *wfield, objstruct *obj )
{
	...
	static double	x2[PSF_NPSFMAX], y2[PSF_NPSFMAX], xy[PSF_NPSFMAX],
					deltax[PSF_NPSFMAX], deltay[PSF_NPSFMAX], flux[PSF_NPSFMAX],
					deltaxb[PSF_NPSFMAX], deltayb[PSF_NPSFMAX],
					fluxb[PSF_NPSFMAX], sol[PSF_NTOT], covmat[PSF_NTOT*PSF_NTOT], 
	...
	if( width < (ival=(int)(psf_fwhm*2)) ) width = ival;
	height = obj->ymax-obj->ymin+1+psf_fwhm;
	if( height < (ival=(int)(psf_fwhm*2)) ) height = ival;
	npix = width*height;
	...
	// copy local image for processing ???
	...
	if( wfield )
	{
		...
		for (wh=weighth, w=weight, dh=datah,p=npix; p--;)
			if( (pix=*(wh++)) < wthresh && pix>0 && (pix2=*(dh++))>-BIG && pix2<satlevel )
				*(w++) = 1/sqrt(pix+(pix2>0.0 ? (gainflag? pix2*pix/backnoise2:pix2)/gain :0.0));
			else
			{
				*(w++) = 0.0;
				wbad++;
	...
	if( wbad>=npix-3 ) return;

	// Weight the data 
	dh = datah;
	// Take into account a local background change
	val = obj->dbkg;      
	d = data;
	w = weight;
	for( p=npix; p--; ) *(d++) = (*(dh++)-val)**(w++);
...
/*---- Subtract previously fitted components */
          ...
          ConvolveImage(field, data2, data3, width,height);
...
/*------ Resample the PSFs here for the 1st iteration */
              VignetResample(psf->maskloc, psf->masksize[0], psf->masksize[1],
                              psfmasks[j], width, height,
                              -deltax[j]*pixstep, -deltay[j]*pixstep,
                              pixstep);       
              m = ComputeGradient(weight,width,height,
          ...
          ComputePos( &npsf, &convflag, &npsfflag,radmin2,radmax2,
  ...
  for (j=0; j<npsf; j++)
    {      
      ...
      if (FLAG(obj2.poserrmx2_psf))
        {
          ComputePosErr(j,var,sol,obj2,x2,y2,xy);
        }
      else
        var += 3*PSF_NA+3;  
  ...
  if (prefs.pc_flag)
    {
      ...
      PcFit(psf, data, weight, width, height, ix,iy, dx,dy, npix,
             field->backsig);
    }
...
// Method:		DoublePsfFit
// Class:		CSextractor
// Purpose:		double fit to make the psf detection on one image and
//				the photometry on another
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::DoublePsfFit( psfstruct *ppsf, picstruct *pfield, 
								picstruct *pwfield, objstruct *obj, 
								psfstruct *psf, picstruct *field, 
													picstruct *wfield)
{
      ...
      if (j>1)
        {
          ...
          ConvolveImage(pfield, pdata2, pdata3, width,height);
         /*---- Ignore regions too close to stellar cores */
          for (k=0; k<j-1; k++)
            {
              pd = pdata3;
...
// Method:		PsfBuild
// Class:		CSextractor
// Purpose:		Build the local PSF (function of "context").
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::PsfBuild( psfstruct *psf )
{
...
// Output:		nothing
////////////////////////////////////////////////////////////////////
double* CSextractor::ComputeGradient( double *weight,int width, int height,
...
////////////////////////////////////////////////////////////////////
// Method:		ComputeGradientPhot
// Class:		CSextractor
// Purpose:		compute gradient phot
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
double* CSextractor::ComputeGradientPhot( double *pweight,int width, 
...
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ComputePos( int *pnpsf, int *pconvflag, int *pnpsfflag,
...
////////////////////////////////////////////////////////////////////
// Method:		ComputePosPhot
// Class:		CSextractor
// Purpose:		compute pos phot
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ComputePosPhot( int *pnpsf,double *sol,double *flux )
{
	int j,npsf;   
	npsf=*pnpsf;
...
////////////////////////////////////////////////////////////////////
// Method:		ComputePosErr
// Class:		CSextractor
// Purpose:		compute poserr
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::ComputePosErr( int j,double *var,double *sol,
								...
								double *y2, double *xy )
{
  ...
  varb = *(var += PSF_NA);
  var += PSF_NA+1;
  obj2arg->poserrmx2_psf = (vara*x2[j]*x2[j]+varb*xy[j]*xy[j]
                         +2*covab*x2[j]*xy[j])/(sol[0]*sol[0]);
  obj2arg->poserrmy2_psf = (varb*y2[j]*y2[j]+vara*xy[j]*xy[j]
                         +2*covab*y2[j]*xy[j])/(sol[0]*sol[0]);
  obj2arg->poserrmxy_psf = (vara*x2[j]*xy[j]+varb*y2[j]*xy[j]
                         +covab*(x2[j]*y2[j]+xy[j]*xy[j]))
    /(sol[0]*sol[0]);
  
  /*------ If requested, translate variances to major and minor error axes... */
  // larry :: here you might wanna use obj2out
  if (FLAG(obj2.poserra_psf))
    {
      double    pmx2,pmy2,temp,theta;
      
      if (fabs(temp=obj2arg->poserrmx2_psf - obj2arg->poserrmy2_psf) > 0.0)
        theta = atan2(2.0 * obj2arg->poserrmxy_psf,temp) / 2.0;
      else
        theta = PI/4.0;
      
      temp = sqrt(0.25*temp*temp+obj2arg->poserrmxy_psf * obj2arg->poserrmxy_psf);
      pmy2 = pmx2 = 0.5*(obj2arg->poserrmx2_psf + obj2arg->poserrmy2_psf);
      pmx2+=temp;
      pmy2-=temp;
      
      obj2arg->poserra_psf = (float)sqrt(pmx2);
      obj2arg->poserrb_psf = (float)sqrt(pmy2);
      obj2arg->poserrtheta_psf = theta*180.0/PI;
    }
  ...
  if (FLAG(obj2.poserr_cxx))
    {
      ...
      xm2 = obj2arg->poserrmx2_psf;
      ym2 = obj2arg->poserrmy2_psf;
      xym = obj2arg->poserrmxy_psf;
      obj2arg->poserrcxx_psf = (float)(ym2/(temp=xm2*ym2-xym*xym));
      obj2arg->poserrcyy_psf = (float)(xm2/temp);
      obj2arg->poserrcxy_psf = (float)(-2*xym/temp);
    }
...
// Method:		SvdFit
// Class:		CSextractor
// Purpose:		General least-square fit A.x = b, based on Singular 
//				Value Decomposition (SVD).Loosely adapted from 
//				Numerical Recipes in C, 2nd Ed. (p. 671).Note: the 
//				a and v matrices are transposed with respect to the 
//				N.R. convention.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::SvdFit( double *a, double *b, int m, int n, 
...
// Method:		SvdVar
// Class:		CSextractor
// Purpose:		Computation of the covariance matrix from the SVD 
//				vmat and wmat matrices. Adapted from Numerical Recipes 
//				in C, 2nd Ed. (p. 679).
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::SvdVar( double *v, double *w, int n, double *cov )
{
	...
	for( i=0; i<n; i++ )
	{
		for( j=0; j<=i; j++ )
		{
			for( sum=0.0,k=0; k<n; k++ ) sum += v[k*n+i]*v[k*n+j]*wti[k];
