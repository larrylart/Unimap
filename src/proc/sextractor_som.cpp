...
// Method:		SomPhot
// Class:		CSextractor
// Purpose:		Perform SOM-fitting on a detected source: returns node
//				number of the best-fitting prototype.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::SomPhot( somstruct *som, float back, float backnoise, 
...
/* First we compute the error map */
  if (clip!=0.0)
    if (!SomMkWeight(som, back, backnoise, gain))
      {
...
*/
  ...
  for (j=0; j<som->nneur; j++)
    {
    if ((err=SomErr(som, (float)j, SOM_NODE))<errmin)
      {
      errmin = err;
      jmin = j;
      }
...
/* Gradient search */
  SomConjgrad(som, 1e-6);
/* Now perform the true photometry */
  som->stderror = (float)sqrt( SomErr(som, 0.0, SOM_PHOTOM));
...
// Output:		nothing
////////////////////////////////////////////////////////////////////
int CSextractor::SomMkWeight( somstruct *som, float back, float backnoise, float gain )
{
...
// Method:		SomErr
// Class:		CSextractor
// Purpose:		Return the reduced RMS error at some (non-integer) 
//				position in the SOM. 1 degree of freedom is left: 
//				the amplitude of the prototype.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
float CSextractor::SomErr( somstruct *som, float dist, int flags )
{
...
// Method:		SomLinMin
// Class:		CSextractor
// Purpose:		Perform minimisation through line-search using two 
//				routines from Numerical Recipes in C: mnbrak() and 
//				brent() (pp. 297 and 301).
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
float CSextractor::SomLinMin( somstruct *som )
{
...
/* Begin by bracketing a minimum of the function */
  ...
  bx = 1.0;
  if ((fb= SomErr(som, bx, SOM_LINE)) > (fa= SomErr(som, ax, SOM_LINE)))
    {
    SHFT(dum, ax, bx, dum);
    SHFT(dum, fb, fa, dum);
    }
  fc = SomErr(som, cx = bx+GOLD*(bx-ax), SOM_LINE);
  while (fb > fc)
    {
    r = (bx-ax)*(fb-fc);
    q = (bx-cx)*(fb-fa);
    ...
    if ((bx-u)*(u-cx) > 0.0)
      {
      if ((fu= SomErr(som, u, SOM_LINE)) < fc)
        {
        ax = bx;
        bx = u;
        fa = fb;
      ...
      fu = SomErr(som, u = cx + GOLD*(cx-bx), SOM_LINE);
      }
      ...
      if ((fu= SomErr(som, u, SOM_LINE)) < fc)
        ...
        SHFT(fb, fc, fu, SomErr(som, u, SOM_LINE));
        }
      }
    else if ((u-ulim)*(ulim-cx) >= 0.0)
      fu = SomErr(som, u=ulim, SOM_LINE);
    else
      fu = SomErr(som, u = cx + GOLD*(cx-bx), SOM_LINE);
    SHFT(ax, bx, cx, u);
    SHFT(fa, fb, fc, fu);
...
/* Now we step to Brent's algorithm for finding the minimum */
  ...
  b = (ax > cx) ? ax : cx;
  x = w = v = bx;
  fw = fv = fx = SomErr(som, x, SOM_LINE);
  for (iter=ITMAX; iter--;)
    {
    xm = 0.5*(a+b);
    ...
    if ((fu= SomErr(som, u, SOM_LINE)) <= fx)
      {
      if (u >= x)
        a = x;
      else
...
// Method:		SomConjgrad
// Class:		CSextractor
// Purpose:		Perform Polak-Ribiere minimization (adapted from 
//				Numerical Recipes in C,p.432).
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::SomConjgrad( somstruct *som, float ftol )
{
  ...
  nd = som->neurdim;
  xi = som->dvector;
  fp = SomErr(som, 0.0, SOM_GRADIENT);
  gt = g;
  xit = xi;
  ht = h;
  for (j=nd;j--;)
...
// Method:		SomEnd
// Class:		CSextractor
// Purpose:		Terminate SOM.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::SomEnd( somstruct *som )
{
	// Free memory*/
	free(som->weight);
	free(som->input);
...
// Method:		SomLoad
// Class:		CSextractor
// Purpose:		Read the SOM weights in a FITS file.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
somstruct* CSextractor::SomLoad( char *filename )
{
   somstruct	*som;
   catstruct	*cat;
   tabstruct	*tab;
