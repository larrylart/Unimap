...
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::PcEnd( pcstruct *pc )
{
   int	i;
...
// Method:		PcLoad
// Class:		CSextractor
// Purpose:		Load the PC data from a FITS file.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
pcstruct* CSextractor::PcLoad( catstruct *cat )
{
   pcstruct	*pc;
   tabstruct	*tab;
   keystruct	*key;
...
/* Load the PC mask data */
  ...
  pc->maskcomp = (float*) key->ptr;

  key = read_key(tab, "PC_MASK");
  pc->omaskcomp = (float*) key->ptr;
  ...
  pc->mx2 = (double*) key->ptr;

  key = read_key(tab, "PC_MY2");
  pc->my2 = (double*) key->ptr;

  key = read_key(tab, "PC_MXY");
  pc->mxy = (double*) key->ptr;

  key = read_key(tab, "PC_FLUX");
  pc->flux = (double*) key->ptr;

  key = read_key(tab, "PC_BRATIO");
  pc->bt = (double*) key->ptr;
...
// Method:		PcFit
// Class:		CSextractor
// Purpose:		Fit the PC data to the current data.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::PcFit( psfstruct *psf, double *data, double *weight,
...
/* Now shift and scale to the right position, and weight the PCs */
  ...
  for (c=npc; c--; cpix += npix)
    {
    VignetResample(cpix, pc->masksize[0], pc->masksize[1],
...
/*- Reconstruct the unconvolved profile */
    ...
    VignetResample(sumopc, pc->omasksize[0], pc->omasksize[1],
