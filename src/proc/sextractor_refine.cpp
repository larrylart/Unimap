...
// Method:		Parcelout
// Class:		CSextractor
// Purpose:		Divide a list of isophotal detections in several parts 
//				(deblending). Even if the object is not deblended, the 
//				output objlist threshold is recomputed if a variable 
//				threshold is used.
// Input:		input objlist, output objlist,
// Output:		RETURN_OK if success, RETURN_FATAL_ERROR otherwise 
//				(memory overflow).
////////////////////////////////////////////////////////////////////
int CSextractor::Parcelout( objliststruct *objlistin, objliststruct *objlistout )
{
   objstruct		*obj;
...
/* ---- initialize lists of objects */
  ...
  memset(objlistRefine, 0, (size_t)xn*sizeof(objliststruct));
  ...
  for (l=0; l<objlistin->nobj && out==RETURN_OK; l++)
      ...
      if ((out = AddObj(l, objlistin, &objlistRefine[0])) == RETURN_FATAL_ERROR)
        goto exit_parcelout;
      if ((out = AddObj(l, objlistin, &debobjlist2)) == RETURN_FATAL_ERROR)
        goto exit_parcelout;
      value0 = objlistRefine[0].obj[0].fdflux*prefs.deblend_mincont;
      ok[0] = (short)1;
      for (k=1; k<xn; k++)
        {
/*------ Calculate threshold */
...
/*--------- Build tree (bottom->up) */
        if (objlistRefine[k-1].nobj>=NSONMAX)
          {
          out = RETURN_FATAL_ERROR;
          goto exit_parcelout;
          }

        for (i=0; i<objlistRefine[k-1].nobj; i++)
          {
          if ((out=Lutz(objlistin,l,&objlistRefine[k-1].obj[i], &debobjlist))
			==RETURN_FATAL_ERROR)
            goto exit_parcelout;

          for (j=h=0; j<debobjlist.nobj; j++)
            if (Belong(j, &debobjlist, i, &objlistRefine[k-1]))
              {
              debobjlist.obj[j].dthresh = debobjlist.dthresh;
              m = AddObj(j, &debobjlist, &objlistRefine[k]);
...
/*------- cut the right branches (top->down) */
      ...
      for (k = xn-2; k>=0; k--)
        {
        obj = objlistRefine[k+1].obj;
        for (i=0; i<objlistRefine[k].nobj; i++)
          {
          ...
          if (m>1)	
			...
			> value0)
                {
                objlistRefine[k+1].obj[j].flag |= OBJ_MERGED	/* Merge flag on */
			| ((OBJ_ISO_PB|OBJ_APERT_PB|OBJ_OVERFLOW)
			&debobjlist2.obj[0].flag);
                if ((out = AddObj(j, &objlistRefine[k+1], &debobjlist2))
			== RETURN_FATAL_ERROR)
                  goto exit_parcelout;
                }
            ok[k+xn*i] = (short)0;
...
exit_parcelout:
      ...
      for (k=0; k<xn; k++)
        {
        free(objlistRefine[k].obj);
        free(objlistRefine[k].plist);
        }
      }
...
////////////////////////////////////////////////////////////////////
// Method:		AllocParcelout
// Class:		CSextractor
// Purpose:		Allocate the memory allocated by global pointers in 
//				refine.c
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::AllocParcelout( void )
{
	QMALLOC(son, short,  prefs.deblend_nthresh*NSONMAX*NBRANCH);
	QMALLOC(ok, short,  prefs.deblend_nthresh*NSONMAX);
	QMALLOC(objlistRefine, objliststruct,  prefs.deblend_nthresh);

	return;
}

////////////////////////////////////////////////////////////////////
// Method:		FreeParcelout
// Class:		CSextractor
// Purpose:		Free the memory allocated by global pointers in refine.c
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
void CSextractor::FreeParcelout( void )
{
	QFREE(son);
	QFREE(ok);
	QFREE(objlistRefine);
...
// Method:		Gatherup
// Class:		CSextractor
// Purpose:		Collect faint remaining pixels and allocate them to 
//				their most probable progenitor.
// Input:		nothing
// Output:		nothing
////////////////////////////////////////////////////////////////////
int CSextractor::Gatherup( objliststruct *objlistin, objliststruct *objlistout )
{
   char		*bmp;
