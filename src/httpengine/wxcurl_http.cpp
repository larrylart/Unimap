...
bool wxCurlHTTP::Options(const wxString& szRemoteFile /*= wxEmptyString*/)
{
	if(m_pCURL)
	{
		...
		SetOpt(CURLOPT_URL, m_szCurrFullURL.ToAscii());
...
////////////////////////////////////////////////////////////////////
bool wxCurlHTTP::PostDF( std::vector<DefFormField>& vectField, const wxString& szRemoteFile /*= wxEmptyString*/)
{
	curl_off_t iSize = 0;
	int i=0;
	...
	if( m_pCURL )
	{
		ResetPostData();
		// add normal fields
		for( i=0; i<vectField.size(); i++ )
		{
			curl_formadd( &m_pPostHead, &m_pPostTail, CURLFORM_COPYNAME, 
							vectField[i].field_name.ToAscii(), (CURLformoption) vectField[i].field_type, 
							vectField[i].field_value.ToAscii() /*c_str()*/, CURLFORM_END );
		}

		// add my file - maybe this should be part of the fields todo: think use ? or have vecto with files separately
//		curl_formadd(&m_pPostHead, &m_pPostTail, CURLFORM_COPYNAME, "upfile", CURLFORM_FILE, strUpFile.c_str(), CURLFORM_END);
...
//		SetOpt(CURLOPT_POSTFIELDSIZE_LARGE, iSize);
		...
		/////////////////
		// larry :: set for file upload
//		struct curl_httppost *post = NULL;
//		struct curl_httppost *last = NULL;
		wxString strMyGroup="Default";
		curl_formadd(&m_pPostHead, &m_pPostTail, CURLFORM_COPYNAME, "group", CURLFORM_COPYCONTENTS, strMyGroup.c_str(), CURLFORM_END);
//		curl_formadd(&m_pPostHead, &m_pPostTail, CURLFORM_COPYNAME, "password", CURLFORM_COPYCONTENTS, sPass.c_str(), CURLFORM_END);
		curl_formadd(&m_pPostHead, &m_pPostTail, CURLFORM_COPYNAME, "upfile", CURLFORM_FILE, strUpFile.c_str(), CURLFORM_END);
...
// Purpose:	This Method Uses Data As Defined By AddForm(...)
////////////////////////////////////////////////////////////////////
...
void wxCurlHTTP::ResetPostData()
{
	...
	// todo: larry check here what it is 
...
void wxCurlHTTP::SetCurlHandleToDefaults()
{
	...
	if(m_bUseCookies)
	{
		...
		SetOpt(CURLOPT_COOKIEJAR, m_szCookieFile.ToAscii());
	}
}
