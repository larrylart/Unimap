 ...
 *   ignored if nm == NULL.
 */
int db_crack_line( char s[], Obj *op, char nm[][MAXNM], int nnm, char whynot[] )
{
 ...
 */
void db_write_line( Obj *op, char lp[] )
{
	switch (op->o_type) {
	case FIXEDOBJ:
	    write_f (op, lp);
...
////////////////////////////////////////////////////////////////////
// Method:	db_tle
// Purpose:	given 3 lines, first of which is name and next 2 are TLE,
//			fill op. we skip leading whitespace on all lines. we do 
//			/not/ assume the 2 TLE lines are 0 terminated, but we do 
//			reach out into each as far as 69 chars. we detect 
//			nonconformance as efficiently as possible. name ends at 
//			first '\0', '\r' or '\n'. 
// Output:	if ok return 0 else return -1
////////////////////////////////////////////////////////////////////
...
// Output:	return 0 if TLE checksum is ok, else -1
////////////////////////////////////////////////////////////////////
int tle_sum( char *l )
{
	char *lastl = l + 68;
...
// Purpose:	extract the given columns and return value.
//			N.B. from and to are 1-based within l
////////////////////////////////////////////////////////////////////
double tle_fld( char *l, int from, int thru )
{
	char buf[32];
...
////////////////////////////////////////////////////////////////////
double wx_tle_fld( wxChar *l, int from, int thru )
{
	wxChar buf[255];

	wxSprintf(buf, wxT("%.*s"), thru-from+1, l+from-1);
	return (wxAtof(buf));
}

// Purpose:	extract the exponential value starting at the given column.
//			N.B. start is 1-based within l
////////////////////////////////////////////////////////////////////
double tle_expfld( char *l, int start )
{
	char buf[32];
	double v;
...
////////////////////////////////////////////////////////////////////
double wx_tle_expfld( wxChar *l, int start )
{
	wxChar buf[255];
	double v;

	wxSprintf( buf, wxT(".%.*s"), 5, l+start );
	v = wxAtof(buf) * pow (10.0, wx_tle_fld(l, start+6, start+7));
	if( l[start-1] == '-' ) v = -v;
...
static int crack_name( Obj *op, char *flds[MAXFLDS], int nf, char nm[][MAXNM], int nnm )
{
	char *sflds[MAXFLDS];
	int nsf;
	int i;
