...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogMStarsCcdm::CSkyCatalogMStarsCcdm( CSkyCatalogMStars* pCatalogMStars, CUnimapWorker* pWorker ) 
{
...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogMStarsCcdm::~CSkyCatalogMStarsCcdm()
{
...
/////////////////////////////////////////////////////////////////////
unsigned long CSkyCatalogMStarsCcdm::Load( int nLoadType, int nLoadSource, int nLoadLimit, 
									  int bRegion, double nCenterRa, double nCenterDec, double nRadius )
{
	// check catalog loaded
	if( m_pCatalogMStars->m_vectData != NULL )
	{
		if( m_pCatalogMStars->m_nCatalogLoaded != CATALOG_ID_CCDM )
	...
	} else if( nLoadType == CATALOG_LOAD_REMOTE )
	{
		if( nLoadSource == CATALOG_QUERY_SOURCE_VIZIER )
		{
			...
			nObj = oRemote.GetMStarsFromVizier( CATALOG_ID_CCDM, bRegion, nCenterRa, nCenterDec, nRadius, nRadius );
		}
	}
...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
unsigned long CSkyCatalogMStarsCcdm::LoadBinary( const wxString& strFile, double nCenterRa, double nCenterDec, 
										double nRadius, int bRegion )
{
	...
	while( !feof( pFile ) )
	{
		...
		if( !fread( (void*) &(m_pCatalogMStars->m_vectData[m_pCatalogMStars->m_nData].comp), sizeof(char), 2, pFile ) )
			break;
		...
		if( !fread( (void*) &(m_pCatalogMStars->m_vectData[m_pCatalogMStars->m_nData].spectral_type), sizeof(char), 3, pFile ) )
			break;
...
// Output:	status
/////////////////////////////////////////////////////////////////////
int CSkyCatalogMStarsCcdm::ExportBinary( DefCatMStars* vectCatalog,
									unsigned int nSize )
{
	FILE* pFile = NULL;
	unsigned long i = 0;
	...
	for( i=0; i<nSize; i++ )
	{
		...
		if( !fwrite( (void*) &(vectCatalog[i].comp), sizeof(char), 2, pFile ) )
			break;
		...
		if( !fwrite( (void*) &(vectCatalog[i].spectral_type), sizeof(char), 3, pFile ) )
			break;
