...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogRadioMsl::CSkyCatalogRadioMsl( CSkyCatalogRadio* pCatalogRadio, CUnimapWorker* pWorker ) 
{
...
// Output:	nothing
/////////////////////////////////////////////////////////////////////
CSkyCatalogRadioMsl::~CSkyCatalogRadioMsl()
{
									  ...
									  int bRegion, double nCenterRa, double nCenterDec, double nRadius )
{
	...
	} else if( nLoadType == CATALOG_LOAD_REMOTE )
	{
		if( nLoadSource == CATALOG_QUERY_SOURCE_VIZIER )
		{
			...
			nObj = oRemote.GetRadioFromVizier( CATALOG_ID_MSL, bRegion, nCenterRa, nCenterDec, nRadius, nRadius );
		}
	}
...
// Method:	LoadBinary
// Class:	CSkyCatalogRadioMsl
// Purpose:	load the binary version of the MSL radio sources catalog
// Input:	if to load region or all
// Output:	nothing
/////////////////////////////////////////////////////////////////////
unsigned long CSkyCatalogRadioMsl::LoadBinary( const wxString& strFile, double nCenterRa, double nCenterDec, 
										double nRadius, int bRegion )
{
	...
	} else
	{
		...
		int nRecords = (int) ( ftell( pFile )/( 14+sizeof(unsigned long)+4*sizeof(double) ));
		// allocate my vector + 1 unit just to be sure
	...
	while( !feof( pFile ) )
	{
		...
		// :: name - 22 chars
		if( !fread( (void*) &(m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].cat_name), sizeof(char), 14, pFile ) )
			break;
		// :: catalog number 
...
*/
		...
		// reset other to zero
		m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].i_flux_density = 0.0;
		m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].no_spec_points = 0.0;
		m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].spec_abscissa = 0.0;
		m_pCatalogRadio->m_vectData[m_pCatalogRadio->m_nData].spec_slope = 0.0;
...
// Method:	ExportBinary
// Class:	CSkyCatalogRadioMsl
// Purpose:	export MSL radio catalog as binary
// Input:	reference vector,size, and file to export to
// Output:	status
/////////////////////////////////////////////////////////////////////
int CSkyCatalogRadioMsl::ExportBinary( DefCatBasicRadio* vectCatalog,
									unsigned int nSize )
{
	FILE* pFile = NULL;
	unsigned long i = 0;
	...
	{
		// :: name - 22 chars
		if( !fwrite( (void*) &(vectCatalog[i].cat_name), sizeof(char), 14, pFile ) )
			break;
		// :: catalog number
