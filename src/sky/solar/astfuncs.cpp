              ...
              const uint32_t  *elem, const double t_epoch)
{
   ...
   mean_anomaly           = DEG2RAD * (double)elem[0] / TEN_MILLION;
   class_elem->asc_node   = DEG2RAD * (double)elem[3] / TEN_MILLION;
   class_elem->arg_per    = DEG2RAD * (double)elem[4] / TEN_MILLION;
   class_elem->incl       = DEG2RAD * (double)elem[5] / TEN_MILLION;
                                    ...
                                    double  *loc, double  *vel )
{
	...
	if( vel && (elem->angular_momentum != 0.))
	{
		...
		double radial_component = elem->ecc * sin( true_anom) * elem->angular_momentum / (r * r0);
                  ...
                  double  *loc, double  *vel )
{
	...
	if( elem->ecc != 1.)    
	{
		...
		// elliptical case;  throw out extra orbits 
...
////////////////////////////////////////////////////////////////////
int  comet_posn( ELEMENTS  *elem, double t, double  *loc )
{
