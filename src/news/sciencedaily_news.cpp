...
////////////////////////////////////////////////////////////////////
int CNewsScienceDaily::Get_Headlines( CNews* pNews )
{
	wxXmlDocument xml;
//	wxString strPage="";
	wxString strUrl;
	...
	// build url
	strUrl = wxT( "http://www.sciencedaily.com/rss/space_time/astronomy.xml" );
	// get page
			...
			while( child2 )
			{
				...
				if( child2->GetName() == wxT("title") )
				{
					...
					else
					{
						...
						elem.title = strTitle;
						elem.source = NEWS_SOURCE_SCIENCEDAILY;
				...
				else if( bFound && child2->GetName() == wxT("pubDate") )
				{
					...
					{
						news_time.MakeFromTimezone(wxDateTime::TZ::EST);
