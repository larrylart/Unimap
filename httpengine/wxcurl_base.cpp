...
extern "C"
{
    ...
    {
		wxString szMessage(cStrMessage, wxConvUTF8, msgSize);
	...
	size_t wxcurl_header_func(void *ptr, size_t size, size_t nmemb, void *stream)
	{
		...
		{
			// tocheck: was char
			(*pStr) += wxString(((char*)ptr),wxConvUTF8);
		}
	...
	size_t wxcurl_str_read(void* ptr, size_t size, size_t nmemb, void* stream)
	{
		...
		if(pStr)
		{
			...
			{
				strncpy((char*)ptr, ((const char*)(*pStr).ToAscii()), iRealSize);
				iRetVal = iRealSize;
			}
			else
			{
				strncpy((char*)ptr, ((const char*)(*pStr).ToAscii()), pStr->Len());
				iRetVal = pStr->Len();
			}
...
bool wxCurlBase::Perform()
{
	...
	res = curl_easy_perform(m_pCURL); // CURLE_READ_ERROR
...
{
	return wxString(m_szErrorBuffer,wxConvUTF8);
}
...
bool wxCurlBase::GetVerboseString(wxString& szStream)
{
	if(m_bVerbose)
	{
		...
		if(pBuff)
		{
			...
			szStream.Append( wxString(pBuff,wxConvUTF8) );
...
void wxCurlBase::SetCurlHandleToDefaults()
{
	if(m_pCURL && ResetHandle())
	{
		...
		if(!m_szUsername.IsEmpty() || !m_szPassword.IsEmpty())
		{
			...
			SetOpt(CURLOPT_USERPWD, m_szUserPass.ToAscii());
		...
		if(m_bUseProxy && !m_szProxyHost.IsEmpty())
		{
			...
			SetOpt(CURLOPT_PROXY, m_szProxyHost.ToAscii());
		}
		...
		if(m_bUseProxy && (!m_szProxyUsername.IsEmpty() || !m_szProxyPassword.IsEmpty()))
		{
			...
			SetOpt(CURLOPT_PROXYUSERPWD, m_szProxyUserPass.ToAscii());
		}
...
void wxCurlBase::SetHeaders()
{
	if(!m_arrHeaders.IsEmpty())
	{
		...
		{
			m_pHeaders = curl_slist_append(m_pHeaders, m_arrHeaders[i].ToAscii());
		}
...
wxDateTime wxCurlBase::GetDateFromString(const wxString& szDate)
{
	...
	return wxDateTime(curl_getdate(szDate.ToAscii(), &now));
}

wxString wxCurlBase::GetURLEncodedString(const wxString& szData)
{
	char* pszRetVal = curl_escape(szData.ToAscii(), szData.Len());

	if(pszRetVal)
	{
		wxString szRetVal(pszRetVal,wxConvUTF8);
...
{
	char* pszRetVal = curl_unescape(szData.ToAscii(), szData.Len());
