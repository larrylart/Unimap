...
{	Entry *p;
	...
	fprintf(fmsg, "Saving %s annotated copy of the input definitions\n", pathsoapStub);
...
{ Entry *Eptr;
  ...
  { fprintf(fheader, "\n\n#ifndef %s", soap_type(typ));
    ...
    fprintf(fd, "\n\n");
  if (is_volatile(typ))
    fprintf(fd, "#if 0 /* volatile type: do not redeclare here */\n");
	  ...
	  { char *sname;
	    ...
	    { Entry *p, *q = entry(t, method->sym);
		  ...
		  { if (p->info.typ == q->info.typ)
		    ...
		    else if (q->info.typ->type == Tpointer && p->info.typ == q->info.typ->ref)
		      fprintf(fd, "\n\t%s%s%s = *%s;", s, a, ident(p->sym->name), ident(q->sym->name));
		    else
		      d = 0;
		  }
  ...
  { for (p = t->list; p; p = p->next)
	...
	if (q && is_transient(q->info.typ))
	  ...
	  if (is_document(method_style))
	    ...
	    fprintf(fd, " <part name=\"%s\" element=\"%s\"/>\n", ns_remove(q->sym->name), ns_add(q->sym->name, ns));
	  else if (is_XML(q->info.typ->ref) || is_stdXML(q->info.typ->ref))
	    fprintf(fd, " <part name=\"parameters\" type=\"xsd:anyType\"/>\n");
	  else
	    fprintf(fd, " <part name=\"%s\" type=\"%s\"/>\n", ns_remove(q->sym->name), wsdl_type(q->info.typ, ns));
	  if (mimeout)
	...
	{ if (is_attachment(p->info.typ))
      ...
      { q = ((Table*)p->info.typ->ref)->list;
		...
		if (q->info.maxOccurs == 1)
	          fprintf(fd, "  <complexType name=\"%s\">\n   <complexContent>\n    <restriction base=\"SOAP-ENC:Array\">\n     <sequence>\n      <element name=\"%s\" type=\"%s\" minOccurs=\"0\" maxOccurs=\"unbounded\" nillable=\"%s\"/>\n     </sequence>\n     <attribute ref=\"SOAP-ENC:arrayType\" WSDL:arrayType=\"%s[%s]\"/>\n    </restriction>\n   </complexContent>\n  </complexType>\n", wsdl_type(p->info.typ, NULL), q->sym->name[5]?ns_remove(q->sym->name+5):"item", wsdl_type(q->info.typ, ns1), nillable(q->info.typ->ref), wsdl_type(q->info.typ, ns1), cbuf);
                else
	          fprintf(fd, "  <complexType name=\"%s\">\n   <complexContent>\n    <restriction base=\"SOAP-ENC:Array\">\n     <sequence>\n      <element name=\"%s\" type=\"%s\" minOccurs=\"%ld\" maxOccurs=\"%ld\" nillable=\"%s\"/>\n     </sequence>\n     <attribute ref=\"SOAP-ENC:arrayType\" WSDL:arrayType=\"%s[%s]\"/>\n    </restriction>\n   </complexContent>\n  </complexType>\n", wsdl_type(p->info.typ, NULL), q->sym->name[5]?ns_remove(q->sym->name+5):"item", wsdl_type(q->info.typ, ns1), q->info.minOccurs, q->info.maxOccurs, nillable(q->info.typ->ref), wsdl_type(q->info.typ, ns1), cbuf);
	    }
          }
	  ...
	  { if (q->info.maxOccurs == 1)
	      ...
	      gen_type_documentation(fd, t, p, ns, ns1);
              fprintf(fd, "   <complexContent>\n    <extension base=\"%s\">\n     <sequence>\n", ns_convert(((Table*)p->info.typ->ref)->prev->sym->name));
	      fprintf(fd, "      <element name=\"%s\" type=\"%s\" minOccurs=\"%ld\" maxOccurs=\"%ld\" nillable=\"%s\"/>\n", q->sym->name[5]?ns_remove(q->sym->name+5):"item", wsdl_type(q->info.typ, ns1), q->info.minOccurs, q->info.maxOccurs, nillable(q->info.typ->ref));
              fprintf(fd, "     </sequence>\n    </extension>\n   </complexContent>\n");
	      gen_schema_attributes(fd, p->info.typ, ns, ns1);
              fprintf(fd, "  </complexType>\n");
	    }
	  ...
	  { if (q->info.maxOccurs == 1)
	    { fprintf(fd, "  <complexType name=\"%s\">", ns_remove(p->sym->name));
	      gen_type_documentation(fd, t, p, ns, ns1);
	      fprintf(fd, "   <sequence>\n    <element name=\"%s\" type=\"%s\" minOccurs=\"0\" maxOccurs=\"unbounded\" nillable=\"%s\"/>\n   </sequence>\n  </complexType>\n", q->sym->name[5]?ns_remove(q->sym->name+5):"item", wsdl_type(q->info.typ, ns1), nillable(q->info.typ->ref));
	    }
            else
	    { fprintf(fd, "  <complexType name=\"%s\">", ns_remove(p->sym->name));
	      gen_type_documentation(fd, t, p, ns, ns1);
...
{ char *s, *t;
	  ...
	  if (q->next->info.typ->ref)
	    gen_schema_elements(fd, q->next->info.typ->ref, ns, ns1);
          fprintf(fd, "    </sequence>\n");
          return 1;
        }
	...
	{ if (((Tnode*)q->info.typ->ref)->type == Tpointer)
	  ...
	  if (((Tnode*)q->info.typ->ref)->type == Tpointer)
            if (q->info.maxOccurs == 1)
	      fprintf(fd, "     <element name=\"%s\" type=\"%s\" minOccurs=\"%ld\" maxOccurs=\"unbounded\" nillable=\"true\"%s/>\n", s, wsdl_type(q->info.typ->ref, ns1), q->info.minOccurs, form);
            else
	      fprintf(fd, "     <element name=\"%s\" type=\"%s\" minOccurs=\"%ld\" maxOccurs=\"%ld\" nillable=\"true\"%s/>\n", s, wsdl_type(q->info.typ->ref, ns1), q->info.minOccurs, q->info.maxOccurs, form);
          else
            if (q->info.maxOccurs == 1)
              fprintf(fd, "     <element name=\"%s\" type=\"%s\" minOccurs=\"%ld\" maxOccurs=\"unbounded\"%s/>\n", s, wsdl_type(q->info.typ->ref, ns1), q->info.minOccurs, form);
	    else
...
{ Service *sp = NULL;
  ...
  { fprintf(fd,"\n\tsoap_serializeheader(soap);");
    ...
    if (pout->info.typ->type == Tarray)
      ...
      fprintf(fd,"\n\t\t || soap_put_%s(soap, &soap_tmp_%s, \"%s\", NULL)", c_ident(pout->info.typ), c_ident(pout->info.typ->ref), ns_convert(pout->sym->name));
    fprintf(fd,"\n\t\t || soap_body_end_out(soap)");
    fprintf(fd,"\n\t\t || soap_envelope_end_out(soap))");
    fprintf(fd,"\n\t\t\t return soap->error;");
    fprintf(fd,"\n\t};");
	  ...
	  { if (((Tnode*)p->info.typ->ref)->type == Tpointer
	     && (((Tnode*)((Tnode*)p->info.typ->ref)->ref)->type == Tclass
	      || ((Tnode*)((Tnode*)p->info.typ->ref)->ref)->type == Tstruct))
              gen_atts(fd, ((Tnode*)((Tnode*)p->info.typ->ref)->ref)->ref, nsa);
...
{ Table *table,*t;
  ...
  { case Tstruct:
	...
	{ if (is_repetition(p))
	  ...
	  else if (p->info.typ->type == Tpointer && is_qname(p->info.typ->ref))
            fprintf(fout,"\n\tconst char *soap_tmp_%s = a->%s ? soap_QName2s(soap, *a->%s) : NULL;", ident(p->sym->name), ident(p->sym->name), ident(p->sym->name));
	  else if (p->info.typ->type == Tpointer && is_stdqname(p->info.typ->ref))
   ...
   {  if (!is_invisible(typ->id->name))
	...
	{ if (p->info.sto & Sreturn)
	  ...
	  { fprintf(fout,"\n\tif (a->%s)", ident(p->next->sym->name));
	    ...
	    else if (is_qname(p->next->info.typ->ref))
