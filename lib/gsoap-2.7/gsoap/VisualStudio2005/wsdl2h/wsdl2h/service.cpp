...
{ // compile the definitions and generate gSOAP header file
  ...
  { banner("Services");
    ...
    fprintf(stream, "- @ref page_notes \"Notes\"\n");
    fprintf(stream, "- @ref page_XMLDataBinding \"XML Data Binding\"\n");
    if (!jflag)
      fprintf(stream, "- @ref SOAP_ENV__Header \"SOAP Header Content\"\n");
    if (!jflag)
      fprintf(stream, "- @ref SOAP_ENV__Detail \"SOAP Fault Detail Content\" (when applicable)\n");
    fprintf(stream, "\n\n*/\n");
  ...
  { if (*op2 && (*op2)->input)
    { bool flag = false, anonymous = ((*op2)->style != document && (*op2)->parameterOrder != NULL);
      ...
      if (!(*op2)->input->header.empty())
        fprintf(stream, "  - Request message has mandatory header part(s) (see @ref SOAP_ENV__Header):\n");
      ...
      { if (!(*op2)->output->header.empty())
          fprintf(stream, "  - Response message has mandatory header part(s): (see @ref SOAP_ENV__Header)\n");
