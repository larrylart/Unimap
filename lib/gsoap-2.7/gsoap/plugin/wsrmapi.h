...
#include "wsaapi.h" /* also includes soapH.h */
...
*/
struct soap_wsrm_data
{ short enabled;	/* 1 = log messages, 0 = do not log messages */
  const char *idname;
  unsigned long idnum;
  struct soap_wsrm_sequence *sequences;
...
{ short handle;		/**< sequence in use by source (has a handle) */
  ...
  ULONG64 num;		/**< current message num (source) last num (dest) */
  enum wsrm__FaultCodes fault;		/**< sequence fault (use when error) */
  enum soap_wsrm_state state;		/**< sequence state */
  struct soap_wsrm_message *messages;	/**< source stores msg content */
...
int soap_wsrm_create(struct soap *soap, const char *to, const char *acksto, const char *wsa_id, soap_wsrm_sequence_handle *seq);
